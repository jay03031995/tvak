...
export default function Nav({ onBook }) {
  ...
  const navLinkStyle = (active) => ({
    ...
    fontSize: 13, fontWeight: 400, fontFamily: 'var(--font-secondary), system-ui, sans-serif',
    textDecoration: 'none', transition: 'all .15s', cursor: 'pointer', border: 'none', background: 'none',
    color: active ? '#844d28' : '#483f37',
  ...
  const megaHeading = { fontSize: 10.5, fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase', fontFamily: 'var(--font-secondary), system-ui, sans-serif', color: '#844d28', marginBottom: 10, display: 'block' }
  const megaLink = { display: 'block', padding: '4px 0', fontSize: 13, fontWeight: 300, color: '#483f37', textDecoration: 'none', lineHeight: 1.6, transition: 'color .15s' }

  return (
    <>
      {/* ── STICKY NAV ── */}
      <header style={{ position: 'sticky', top: 0, zIndex: 200, background: '#fff', borderBottom: '1px solid rgba(132,77,40,0.1)', fontFamily: 'var(--font-secondary), system-ui, sans-serif' }}>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: '0 20px', height: 70, display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 8 }} className="nav-desktop">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }} onMouseLeave={closeMegaSoon}>
            ...
            <button onClick={() => { setMenuOpen(o => !o); setSearchOpen(false); setHoverMenu('') }}
              style={{ width: 40, height: 40, borderRadius: '50%', background: '#844d28', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginRight: 6 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }} onMouseLeave={closeMegaSoon}>
            ...
            <div style={{ width: 1, height: 20, background: 'rgba(132,77,40,0.12)', margin: '0 4px' }} />
            {/* Phone */}
            <a href="tel:09811997993" style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#844d28', textDecoration: 'none', fontSize: 13, fontWeight: 400, whiteSpace: 'nowrap' }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#b8894a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13.1a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 2.48l3-.1a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6.08 6.08l1.79-1.79a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
              +91 98119 97993
            </a>
            {/* Book button */}
            <button onClick={onBook}
              style={{ background: '#b8894a', color: '#844d28', fontSize: 13, fontWeight: 600, padding: '9px 22px', borderRadius: 999, border: 'none', cursor: 'pointer', flexShrink: 0, letterSpacing: '0.02em' }}>
              Book Appointment
            </button>
            {/* Search */}
            <button onClick={() => { setSearchOpen(o => !o); setHoverMenu('') }}
              style={{ width: 34, height: 34, borderRadius: '50%', border: '1.5px solid rgba(132,77,40,0.15)', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#844d28' }}>
        ...
        <div style={{ display: 'none', padding: '0 16px', height: 60, alignItems: 'center', justifyContent: 'space-between' }} className="nav-mobile">
          <button onClick={() => { setMenuOpen(o => !o); setSearchOpen(false) }}
            style={{ width: 38, height: 38, borderRadius: '50%', background: '#844d28', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round"><path d="M3 6h18M3 12h18M3 18h18"/></svg>
          </button>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => { setSearchOpen(o => !o); setMenuOpen(false) }}
              style={{ width: 38, height: 38, borderRadius: '50%', border: '1.5px solid rgba(132,77,40,0.2)', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#844d28' }}>
            ...
            <button onClick={onBook}
              style={{ background: '#b8894a', color: '#844d28', fontSize: 12, fontWeight: 600, padding: '8px 16px', borderRadius: 999, border: 'none', cursor: 'pointer' }}>
              Book
            </button>
          </div>
        </div>
        ...
        {/* Search bar */}
        {searchOpen && (
          <div style={{ borderTop: '1px solid rgba(132,77,40,0.08)', background: '#fff' }}>
            <div style={{ maxWidth: 640, margin: '8px auto', padding: '0 20px', display: 'flex', alignItems: 'center', gap: 10, background: '#f5e6d0', borderRadius: 12 }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(72,63,55,0.65)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/></svg>
              <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Search treatments, concerns…"
                style={{ flex: 1, background: 'none', border: 'none', outline: 'none', padding: '10px 0', fontSize: 14, color: '#483f37', fontWeight: 300 }} autoFocus />
              <button onClick={() => { setSearchOpen(false); setSearchQ('') }} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'rgba(72,63,55,0.65)', display: 'flex', padding: 4 }}>
      ...
      <div style={{ position: 'fixed', left: 0, right: 0, zIndex: 190, top: 70, opacity: hoverMenu === 'conditions' ? 1 : 0, pointerEvents: hoverMenu === 'conditions' ? 'auto' : 'none', transform: hoverMenu === 'conditions' ? 'translateY(0)' : 'translateY(-8px)', transition: 'opacity .2s, transform .22s' }}
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid rgba(72,63,55,0.08)', boxShadow: '0 16px 48px rgba(72,63,55,0.1)' }}>
              ...
              <div key={col.heading}>
                ...
                {col.items.map(item => (
                  <Link key={item.name} href={item.href} onClick={closeMega}
                    style={megaLink}
                    onMouseEnter={e => e.currentTarget.style.color = '#b8894a'}
                    onMouseLeave={e => e.currentTarget.style.color = '#483f37'}>
                    {item.name}
                  </Link>
                ))}
              </div>
            ))}
            <div style={{ gridColumn: '1 / -1', borderTop: '1px solid rgba(72,63,55,0.07)', marginTop: 4, paddingTop: 14, display: 'flex', justifyContent: 'flex-end' }}>
              <Link href="/concerns" onClick={closeMega} style={{ fontSize: 12.5, fontWeight: 400, color: '#844d28', textDecoration: 'none' }}>
                View all concerns →
              </Link>
            </div>
          </div>
      ...
      <div style={{ position: 'fixed', left: 0, right: 0, zIndex: 190, top: 70, opacity: hoverMenu === 'treatments' ? 1 : 0, pointerEvents: hoverMenu === 'treatments' ? 'auto' : 'none', transform: hoverMenu === 'treatments' ? 'translateY(0)' : 'translateY(-8px)', transition: 'opacity .2s, transform .22s' }}
        onMouseEnter={keepMega} onMouseLeave={closeMegaSoon}>
        <div style={{ background: '#fff', borderBottom: '1px solid rgba(72,63,55,0.08)', boxShadow: '0 16px 48px rgba(72,63,55,0.1)', maxHeight: 'calc(100vh - 80px)', overflowY: 'auto' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 20px 32px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 32 }}>
            ...
            <div style={{ gridColumn: '1 / -1', borderTop: '1px solid rgba(72,63,55,0.07)', marginTop: 4, paddingTop: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Link href="/treatments" onClick={closeMega} style={{ fontSize: 12.5, fontWeight: 400, color: '#844d28', textDecoration: 'none' }}>
                View all treatments →
              </Link>
              <button onClick={() => { closeMega(); onBook() }} style={{ fontSize: 12.5, fontWeight: 500, padding: '8px 18px', borderRadius: 999, background: '#844d28', color: '#fff', border: 'none', cursor: 'pointer' }}>
                Book Consultation
              </button>
            </div>
          </div>
      ...
      <div style={{ position: 'fixed', left: 0, right: 0, zIndex: 190, top: 70, opacity: hoverMenu === 'clinics' ? 1 : 0, pointerEvents: hoverMenu === 'clinics' ? 'auto' : 'none', transform: hoverMenu === 'clinics' ? 'translateY(0)' : 'translateY(-8px)', transition: 'opacity .2s, transform .22s' }}
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid rgba(72,63,55,0.08)', boxShadow: '0 16px 48px rgba(72,63,55,0.1)' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 20px 28px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
            {CLINICS.map(item => (
              <Link key={item.name} href={item.href} onClick={closeMega}
                style={{ ...megaLink, padding: '8px 12px', borderRadius: 8, background: '#efdfc8', border: '1px solid rgba(72,63,55,0.07)', fontWeight: 400, fontSize: 13.5 }}
                onMouseEnter={e => { e.currentTarget.style.background = '#f5e6d0'; e.currentTarget.style.color = '#844d28' }}
                onMouseLeave={e => { e.currentTarget.style.background = '#efdfc8'; e.currentTarget.style.color = '#483f37' }}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        <div style={{ position: 'fixed', top: 64, left: 0, right: 0, bottom: 0, zIndex: 300, background: '#fff', overflowY: 'auto' }}>
          <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {[
              ...
              <div key={section.key} style={{ borderBottom: '1px solid rgba(72,63,55,0.06)' }}>
                <button onClick={() => setMobileExpanded(mobileExpanded === section.key ? '' : section.key)}
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 400, color: '#483f37' }}>{section.label}</div>
                    <div style={{ fontSize: 12, fontWeight: 300, color: 'rgba(72,63,55,0.65)', marginTop: 2 }}>{section.sub}</div>
                  </div>
                      ...
                      <div key={col.heading} style={{ marginBottom: 14 }}>
                        ...
                        {col.items.map(item => (
                          <Link key={item.name} href={item.href} onClick={() => setMenuOpen(false)}
                            style={{ display: 'block', padding: '6px', fontSize: 13.5, fontWeight: 300, color: '#844d28', textDecoration: 'none' }}>
                            {item.name}
                          </Link>
                        ))}
                      </div>
            ...
            {[
              ...
              <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 12px', textDecoration: 'none', borderBottom: '1px solid rgba(72,63,55,0.06)' }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 400, color: '#483f37' }}>{item.label}</div>
                  <div style={{ fontSize: 12, fontWeight: 300, color: 'rgba(72,63,55,0.65)', marginTop: 2 }}>{item.sub}</div>
                </div>
            ...
            <button onClick={() => { setMenuOpen(false); onBook() }}
              style={{ marginTop: 12, width: '100%', padding: '14px', borderRadius: 999, background: '#844d28', color: '#fff', fontSize: 13, fontWeight: 500, border: 'none', cursor: 'pointer' }}>
              Book Appointment
            </button>
          </div>
        </div>
