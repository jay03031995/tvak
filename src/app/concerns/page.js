...
const FALLBACK = [
  ...
  { name: 'Pigmentation & Melasma', slug: { current: 'pigmentation' }, category: 'Skin & Face', iconBg: '#f5e6d0' },
  ...
  { name: 'Hair Regrowth', slug: { current: 'hair-regrowth' }, category: 'Hair & Scalp', iconBg: '#f5e6d0' },
  { name: 'Unwanted Body Hair', slug: { current: 'unwanted-hair' }, category: 'Hair & Scalp', iconBg: '#FFF0EE' },
  { name: 'Wrinkles & Fine Lines', slug: { current: 'wrinkles' }, category: 'Anti-Ageing', iconBg: '#f5e6d0' },
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
                  {filtered.map((c, i) => (
                    <Link key={i} href={`/concerns/${c.slug?.current || '#'}`} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 12, padding: '12px', background: '#fff', borderRadius: 14, border: '1.5px solid rgba(26,17,9,0.09)', transition: 'transform .2s, box-shadow .2s', minHeight: 72 }}>
                      <span style={{ width: 48, height: 48, borderRadius: 10, background: c.iconBg || '#f5e6d0', flexShrink: 0, overflow: 'hidden', position: 'relative', display: 'block' }}>
                        {(c.image || c.heroImage) ? (
                          <Image
