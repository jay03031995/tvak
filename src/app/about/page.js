...
export default async function AboutPage() {
  ...
  return (
    <div style={{ background: 'var(--cream)' }}>
      <section style={{ padding: '72px 20px', background: 'linear-gradient(160deg,#f5e6d0,#f5e6d0)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h1 style={{ fontWeight: 500, marginBottom: 16 }}>
            ...
            <em style={{ fontStyle: 'italic', fontWeight: 400, color: '#b8894a' }}>{page?.hero?.headingItalic || 'care.'}</em>
          </h1>
      ...
      <section style={{ padding: '56px 20px', background: '#844d28' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }} className="grid-stats">
          {(page?.stats || [
            ...
            <div key={i}>
              <div style={{ fontSize: 'clamp(28px,3.5vw,40px)', fontWeight: 500, color: '#f5e6d0', lineHeight: 1 }}>{s.value}</div>
              <div style={{ fontSize: 12, fontWeight: 300, color: '#b8894a', marginTop: 8 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
            {values.map((v, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: 16, padding: '28px 26px', border: '1.5px solid rgba(26,17,9,0.09)' }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: '#f5e6d0', marginBottom: 18 }} />
                <h3 style={{ fontWeight: 500, fontSize: 15.5, marginBottom: 10 }}>{v.title}</h3>
                <p style={{ fontSize: 13.5, fontWeight: 300, color: '#483f37', lineHeight: 1.75 }}>{v.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '72px 20px', background: '#f5e6d0' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', alignItems: 'start' }} className="grid-hero">
          ...
          <div>
            ...
            <p style={{ fontSize: 13.5, fontWeight: 400, color: '#b8894a', marginBottom: 18 }}>
            ...
            <Link href="/doctor" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 400, color: '#fff', background: '#844d28', padding: '12px 26px', borderRadius: 999, textDecoration: 'none' }}>
      ...
      <section style={{ padding: '64px 20px', background: '#1a1512' }}>
        <div style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontWeight: 500, color: '#f5e6d0', marginBottom: 10 }}>{locationHeading}</h2>
          <p style={{ fontSize: 13.5, fontWeight: 300, color: '#c8b09a', marginBottom: 28, lineHeight: 1.75 }}>
            {locationAddress}
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href={directionsUrl} target="_blank" rel="noopener"
              style={{ background: '#844d28', color: '#fff', fontSize: 13, fontWeight: 400, padding: '12px 28px', borderRadius: 999, textDecoration: 'none' }}>
              {directionsCta}
            </a>
            <Link href="/contact" style={{ background: 'rgba(255,255,255,0.1)', color: '#f5e6d0', fontSize: 13, fontWeight: 400, padding: '12px 28px', borderRadius: 999, textDecoration: 'none', border: '1px solid rgba(255,255,255,0.15)' }}>
              {bookingCta}
            </Link>
          </div>
        </div>
